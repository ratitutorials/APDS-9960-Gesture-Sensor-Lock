...
bool SparkFun_APDS9960::wireWriteByte(uint8_t val)
{
    ...
    //Wire.write(val);
	uint8_t ptrTemp[1] = { val };

	i2cwire.transmit(1, ptrTemp, APDS9960_I2C_ADDR);
...
bool SparkFun_APDS9960::wireWriteDataByte(uint8_t reg, uint8_t val)
{
    ...
    //Wire.write(reg);
    //Wire.write(val);
	uint8_t ptrTemp[2] = { reg, val };
    i2cwire.transmit(2, ptrTemp, APDS9960_I2C_ADDR);
...
bool SparkFun_APDS9960::wireReadDataByte(uint8_t reg, uint8_t &val)
{
	...
	i2cwire.receiveByte(&val, reg, APDS9960_I2C_ADDR);
    return true;
}
                                        ...
                                        unsigned int len)
{
    ...
    i2cwire.receiveBlock(len, val, reg, APDS9960_I2C_ADDR);
